...
export default function Example() {
  ...
  const [items, setItems] = React.useState([
    { header: true, rodada: 4, serie: "COPA VICTROLA 2020", nameHome: "", backgroundColorHome: "#008000", textColorHome: "#008000", pointsColorHome: "#008000", idHome: 0, ptsHome: 10001, nameAway: "", backgroundColorAway: "#008000", textColorAway: "#008000", pointsColorAway: "#008000", idAway: 0, ptsAway: 10001, rodada_atual: 0 },
                    ...
                    {!item.header && item.pegouAtletasHome && item.pegouAtletasAway ? (item.atletasHome.map(x =>
                      <View style={{ flexDirection: 'row' }}>
                        ...
                        <View style={[styles.itemContainer, { borderWidth: 1, borderColor: item.textColorAway, marginHorizontal: Dimensions.get('window').width * 0.003, backgroundColor: item.backgroundColorAway, width: Dimensions.get('window').width * 0.14 }]}>
                          <Text adjustsFontSizeToFit={true} numberOfLines={2} style={[styles.itemName, { color: item.pointsColorAway }]}>{item.atletasAway[item.atletasHome.indexOf(x)].status_id != 0 ? item.capitaoIdHome == item.atletasAway[item.atletasHome.indexOf(x)].atleta_id ? '(' + item.atletasAway[item.atletasHome.indexOf(x)].pontos_num + ' x2) ' + item.atletasAway[item.atletasHome.indexOf(x)].pontos_num * 2 : item.atletasAway[item.atletasHome.indexOf(x)].pontos_num : '-'}</Text>
                        </View>
