    ...
    if (status == 2) {
      ...
      items.forEach(async element => {
        ...
        if (element.idHome != 0 && element.idAway != 0 && element.rodada == items[0].rodada_atual && !element.header) {
          await generalCache.set(element.idHome + element.serie, element.idAway)
          await generalCache.set(element.idAway + element.serie, element.idHome)
          // try {
      ...
      await generalCache.clearAll()
      items.forEach(async element => {
        if (element.idHome != 0 && element.idAway != 0 && !element.header && element.rodada == items[0].rodada_atual) {
          // console.log(element.nameHome + " x " + element.nameAway)
          // console.log(element.idHome + " - " + await generalCache.get(element.idHome) + " - home")
          // console.log(element.idAway + " - " +await generalCache.peek(element.idAway) != undefined + " - away")



          
          // setTimeout(async function () {
          //   var elementFromCacheHome = await generalCache.peek(element.idHome);
          //   var elementToAddHome = (elementFromCacheHome == undefined ? "" : elementFromCacheHome + "-") + element.idAway   
          //   elementToAddHome = [...new Set(elementToAddHome.split("-"))]         
          //   await generalCache.set(element.idHome, elementToAddHome) 
          //   // console.log([...new Set(elementFromCacheHome.split("-"))])
          // }, 500)

          // setTimeout(async function () {
          //   var elementFromCacheAway = await generalCache.peek(element.idAway);
          //   var elementToAddAway = (elementFromCacheAway == undefined ? "" : elementFromCacheAway + "-") + element.idHome
          //   elementToAddAway = [...new Set(elementToAddAway.split("-"))]
          //   console.log(elementToAddAway)
          //   await generalCache.set(element.idAway, elementToAddAway)          
          // }, 1000)
          
          setTimeout(async function () {
            var elementFromCacheHome = await generalCache.peek(element.idHome);
            await generalCache.set(element.idHome, [...new Set(((elementFromCacheHome == undefined ? "" : elementFromCacheHome + "-") + element.idAway).split("-"))])             
          }, 500)

          setTimeout(async function () {
            var elementFromCacheAway = await generalCache.peek(element.idAway);
            await generalCache.set(element.idAway, [...new Set(((elementFromCacheAway == undefined ? "" : elementFromCacheAway + "-") + element.idHome).split("-"))])          
          },1000)   
          ...
          // console.log(await generalCache.get(element.idAway))
          // if(await generalCache.peek(element.idHome) == undefined){
          //   team = await generalCache.get(element.idHome)
          //   // console.log("home");
          //   // console.log(element.nameHome);
          //   await generalCache.set(element.idHome, team + "-" + element.idAway)
          // } else {
          //   await generalCache.set(element.idHome, element.idAway)
          // }     

          // if(await generalCache.peek(element.idAway) == undefined){
          //   team = await generalCache.get(element.idAway)
          //   // console.log("away")
          //   // console.log(element.nameAway)
          //   await generalCache.set(element.idAway, team + "-" + element.idHome)
          // } else {
          //   await generalCache.set(element.idAway, element.idHome)
          // }
